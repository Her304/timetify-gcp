...
Use `logger = logging.getLogger(__name__)`, never `print()`. `start_local.md` is gitignored, never include anyinfo that belongs to gitignore files . The **committed test suite** lives in `backend/main/tests/` — run it with `python manage.py test main.tests --settings=backend.test_settings` (offline: in-memory SQLite, locmem email; pass `BACK_SENTRY_DSN=` empty so a red test can't page prod). CI (`.github/workflows/ci.yml`) runs it on every push. One-off local scratch scripts (`test_*.py`, `poc_*.py`) at the **repo root** remain gitignored — never commit those. Work style: show thinking before acting, plan before changes (wait for go-ahead), explain *why* not *what*.
